  ...
  describe 'on save' do
    describe ':name' do
      it 'creates an error if the name length is not between 3 and 40 characters' do
        ...
        name_too_long = Shop.create(:name => 'This is my Shop Name, and it is way too long' * 10)
        ...
        expect(error_message_for(name_too_long, :name)).to include('is too long (maximum is 200 characters)')
      ...
      it 'creates an error if name begins or ends in whitespace' do
        ...
        expect(error_message_for(name_whitespace_start, :name)).to include('must not begin or end with whitespace')
        expect(error_message_for(name_whitespace_end, :name)).to include('must not begin or end with whitespace')
        expect(error_message_for(name_whitespace_only, :name)).to include('must not begin or end with whitespace')
        expect(error_message_for(name_valid, :name)).to be_empty
      end
    end
    ...
    describe ':subdomain' do
      it 'creates an error if the subdomain length is not between 3 and 25 characters' do
        ...
        subdomain_too_long = Shop.create(:subdomain => 'this-is-my-subdomain-name-which-is-way-too-long' * 2)
        ...
        expect(error_message_for(subdomain_too_long, :subdomain)).to include('is too long (maximum is 50 characters)')
      ...
      it 'creates an error if subdomain contains characters other than A-z, 0-9, and -' do
        ...
        expect(error_message_for(subdomain_with_underscore, :subdomain)).to include('may only contain alphanumeric characters')
        expect(error_message_for(subdomain_with_space, :subdomain)).to include('may only contain alphanumeric characters')
        expect(error_message_for(subdomain_with_punctuation, :subdomain)).to include('may only contain alphanumeric characters')
        expect(error_message_for(subdomain_valid, :subdomain)).to be_empty
      end
